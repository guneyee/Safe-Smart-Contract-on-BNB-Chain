...
contract CarRentalPlatform is ReentrancyGuard {
  ...
  function addUser(string calldata name, string calldata lastname) external { 
    ...
    usersMapping[msg.sender] = User(msg.sender, name, lastname, 0, 0, 0, 0);
    emit UserAdded(msg.sender, usersMapping[msg.sender].name, usersMapping[msg.sender].lastname);
  }
  ...
  //editdotMetadata #onlyOwner #existingCar
  ...
  function editCarStatus(uint id, Status status) external onlyOwner {
   ...
   //checkOut #existingUser #isCarAvailable #userHas Not RentedACar #userHasNoDebt 
  ...
  //checkIn #existingUser #userHasparkedaLot
